...
with open('server_private_key.bin', 'rb') as priv_ser_key, open('client_public_key,bin', 'rb') as pub_cli_key:
        private_server_key = serialization.load_pem_private_key(
                             priv_ser_key.read(),
                             password=b'1234',
                             backend=default_backend()
                            )
        public_client_key = serialization.load_pem_public_key(
                             pub_cli_key.read(),
                             backend=default_backend()
                            )
...
class ServerWorker(object):
    ...
    def process(self, msg):
        ...
        if(self.msg_cnt == 0):
            ...
            client_key=load_pem_public_key( msg[256:],default_backend())
            ...
            signature = private_server_key.sign(
                         new_msg,
                         padding.PSS(
                             mgf=padding.MGF1(hashes.SHA256()),
                             salt_length=padding.PSS.MAX_LENGTH
